...
g=sns.catplot(data=sumdataset, kind="bar", x="size", y="time", hue="name", ci="sd", palette="dark",height=6, aspect=3).despine(left=True).set(ylim=(0,1000))
ax = g.axes[0,0]
...
sns.set_theme(style="whitegrid")
g=sns.catplot(data=sumdataset, kind="bar", x="size", y="summation_time", hue="name", ci="sd", palette="dark",height=6, aspect=3).despine(left=True)
ax = g.axes[0,0]
for c in ax.containers:
    ax.bar_label(c)
plt.tight_layout()
g.savefig("/home/dzc/Desktop/out1.png")
